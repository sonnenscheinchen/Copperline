...
#[test]
fn test_extb_is_illegal_on_68000() {
    ...
    // EXTB.L D0 (0x49C0) - 68020+ only
    bus.write_word_at(0x0200, 0x49C0);
    ...
    assert_eq!(
        cpu.pc, 0x0300,
        "EXTB.L on 68000 should trap to illegal instruction vector"
    );
}

#[test]
fn test_pack_unpk_are_illegal_on_68000() {
    let mut cpu = CpuCore::new();
    cpu.set_cpu_type(CpuType::M68000);
    let mut bus = TestBus::new();
    ...
    // PACK D0,D1,#0 (0x8340) - 68020+ only
    bus.write_word_at(0x0200, 0x8340);
    bus.write_word_at(0x0202, 0x0000);
    // UNPK D0,D1,#0 (0x8380) - 68020+ only
    bus.write_word_at(0x0204, 0x8380);
    bus.write_word_at(0x0206, 0x0000);
    ...
    assert_eq!(
        cpu.pc, 0x0300,
        "PACK/UNPK on 68000 should trap to illegal instruction vector"
    );
}
