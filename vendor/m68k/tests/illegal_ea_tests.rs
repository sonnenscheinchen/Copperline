...
#[test]
fn test_move_byte_to_address_register_is_illegal() {
    ...
    // MOVE.B D0, A0 (illegal: byte size to address register)
    bus.write_word_at(0x0200, 0x1040);
    ...
    assert_eq!(
        cpu.pc, 0x0300,
        "MOVE.B to address register should trap to illegal instruction vector"
    );
}
