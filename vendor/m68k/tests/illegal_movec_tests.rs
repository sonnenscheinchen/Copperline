...
#[test]
fn test_movec_is_illegal_on_68000() {
    ...
    // MOVEC Rc,Rn (0x4E7A) is 68010+ only.
    // Extension word: D0 (reg_type=0, reg_num=0) and VBR (0x801).
    bus.write_word_at(0x0200, 0x4E7A);
    bus.write_word_at(0x0202, 0x0801);

    cpu.reset(&mut bus);
    cpu.pc = 0x0200;
    cpu.set_sr(0x2700); // supervisor mode to avoid privilege masking the illegal
    ...
    assert_eq!(
        cpu.pc, 0x0300,
        "MOVEC on 68000 should trap to illegal instruction vector"
    );
}
