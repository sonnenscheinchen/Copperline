...
#[test]
fn test_movec_iacr_is_illegal_on_68030() {
    ...
    // MOVEC IACR0, D0 (control reg 0x00A) should be illegal on 68030
    bus.write_word_at(0x0200, 0x4E7A);
    bus.write_word_at(0x0202, 0x000A);
    ...
    assert_eq!(
        cpu.pc, 0x0300,
        "MOVEC IACR0 on 68030 should trap to illegal instruction vector"
    );
}

#[test]
fn test_movec_tc_is_illegal_on_68030() {
    let mut cpu = CpuCore::new();
    cpu.set_cpu_type(CpuType::M68030);
    let mut bus = TestBus::new();
    ...
    // MOVEC TC, D0 (control reg 0x003) should be illegal on 68030
    bus.write_word_at(0x0200, 0x4E7A);
    bus.write_word_at(0x0202, 0x0003);
    ...
    assert_eq!(
        cpu.pc, 0x0300,
        "MOVEC TC on 68030 should trap to illegal instruction vector"
    );
}
